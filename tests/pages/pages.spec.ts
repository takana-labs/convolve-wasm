...
const deployments = [
  { name: "domain root", basePath: "/" },
  {
    name: "transferred repository subpath",
    basePath: "/transferred-owner/renamed-repo/",
  },
];

for (const deployment of deployments) {
  test(
    `loads the complete application site from the ${deployment.name}`,
    async ({ page }, testInfo) => {
      ...
      try {
        ...
        for (const path of runtimePaths) {
          expect(path.startsWith(`${deployment.basePath}assets/`)).toBe(true);
        }
